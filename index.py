...
def autenticar():
    ...
    if (flask.request.method == "POST"):
        ...
        archivo=Archivo()
        archivo.leerBase()
        encontrado=archivo.buscarContacto(co=usuario) #devuelve True ó False de acuerdo a si encuentra el  usuario
        if encontrado=='si':
            pos=3
            contrasenabase=archivo.datoposContacto(co=usuario,pos=pos)
...
#        contrasena2 = flask.request.form["contrasena2"]
        ...
        aval_club='SI'     #validar los avales mas adelante
        tipo_usuario='1'
        fecha=date.today()
        fecha=str(fecha)
...
def clubcreaagendadia():#CREAR DIA Agenda para un campo específico
    if(flask.request.method == "POST"):
        ...
        pos=5
        usuario=flask.session["username"]
        club=archivo.datoposContacto(co=usuario,pos=pos) #recupera nombre del Club y lo asigna a varibale club
        course=Campos()
        course.leerCampos()
...
def inicioagendajugador():
    ...
    co=flask.session["username"]
    pos=5
    club=archivo.datoposContacto(co=co,pos=pos)
